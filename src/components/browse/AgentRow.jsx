...
export function AgentRow({ agent, onTagClick }) {
  ...
  const filterTag = (label) => { onTagClick(label); };
  const onKey = (e) => { if (e.key === 'Enter') open(); };

  return (
    <li className="agent-item" tabIndex={0} onKeyDown={onKey}>
      <a className="agent-item-link" onClick={open} role="button" aria-label={`View ${agent.name}`}>
        ...
        <div className="agent-item-tags">
          {agent.caps.slice(0, 3).map((t) => (
            <span key={t} className="agent-tag" onClick={(e) => { e.stopPropagation(); filterTag(t); }}>{t}</span>
          ))}
          {agent.models.slice(0, 2).map((t) => (
            <span key={t} className="agent-tag" onClick={(e) => { e.stopPropagation(); filterTag(t); }}>{t}</span>
          ))}
        </div>
