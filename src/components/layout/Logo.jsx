/** AgentHub wordmark + icon. */
import { Link } from 'react-router-dom';

export function Logo({ compact = false, onClick }) {
  return (
    <Link to="/" className={`logo${compact ? ' logo--compact' : ''}`} aria-label="AgentHub home" onClick={onClick}>
      ...
      {!compact && <span className="logo-text">AgentHub</span>}
    </Link>
  );
}
